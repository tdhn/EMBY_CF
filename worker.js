// ================================================================================
// 🔥 Wrangler 3.9x 兼容 V4 格式修正版（仅修改格式，原有逻辑100%保留）
// ================================================================================
export default {
...
<body>
  <h1>Cloudflare Worker is running</h1>
</body>
</html>
`;
...
async function handleStatsRequest(env) {
  try {
    if (!env.DB) return new Response(JSON.stringify({ status: "running" }), { headers: { 'Content-Type': 'application/json' } });
    const results = await env.DB.prepare("SELECT * FROM auto_emby_daily_stats ORDER BY date DESC LIMIT 7").all();
    return new Response(JSON.stringify({ status: "running", stats: results.results }), { headers: { 'Content-Type': 'application/json' } });
  } catch (e) {
